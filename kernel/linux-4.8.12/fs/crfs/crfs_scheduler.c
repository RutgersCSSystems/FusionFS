...
static inline struct crfss_fstruct* crfss_sched_pick_next
	...
	switch (policy) {
		...
		case DEVFS_CFS_SCHED:
			return devfs_sched_round_robin_cfs(dev_thread_ctx, SCHED_CPU);
		default:
			return crfss_sched_round_robin(dev_thread_ctx);
	}
}
...
#endif
                /* Process compound I/O request */
                vfio_process_compound(rd, cmdrw);
...
static int crfss_add_per_thread_list(struct crfss_fstruct *rd) {
	...
	crfss_mutex_lock(&g_mutex);	
	//index = g_dev_thread_round;
	//g_dev_thread_round = (g_dev_thread_round + 1) % devfs_device_thread_nr;
	...
	for (i = 0; i < crfss_device_thread_nr; ++i) {
        ...
        /* printk(KERN_ALERT "DEBUG: insert per-thread rd list, fd: %d, core: %d\n", rd->fd, index); */
	crfss_mutex_unlock(&g_mutex);	
