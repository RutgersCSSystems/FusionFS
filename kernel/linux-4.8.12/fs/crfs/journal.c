		...
		crfs_transaction_t *trans)
{
	...
	if (trans->is_micro_trans == true)
		return crfs_macro_replay_transaction(sb, trans, true);
#endif
...
#ifdef _MACROFS_JOURN
	if (trans->is_micro_trans == true)
		return crfs_macro_replay_transaction(sb, trans, recover);
#endif
...
#ifdef _MACROFS_JOURN
			if (le->type & LE_MACRO) {
				//printk(KERN_ALERT "%s:%d getting called\n", __FUNCTION__, __LINE__);
				crfs_macro_recover_trans(&trans, le);
			}
#endif
...
//#if 0 //ndef _DEVFS_JOURN
#ifndef _MACROFS_JOURN
        return trans;
#endif
...
int crfs_add_logentry_data(struct super_block *sb,
