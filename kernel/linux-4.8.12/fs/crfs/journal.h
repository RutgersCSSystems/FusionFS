...
typedef struct crfs_transaction {
	...
	u16              status;
	u8               num_micro_op;
	char             mop_bitmap[MACRO_BITMAP_SIZE];
	char             mop_commit_bitmap[MACRO_BITMAP_SIZE];
	bool             is_micro_trans;
...
typedef struct crfs_micro_op_data {
	__le64  mop_bitmap;
	__le64  mop_commit_bitmap;
	__le16  mop_ptr[16];
} crfs_micro_op_data;
...
extern int crfs_add_logentry_data(struct super_block *sb,
		crfs_transaction_t *trans, void *addr, uint16_t size, void *data, u8 type);
...
extern void crfs_macro_recover_trans(crfs_transaction_t *trans, crfs_logentry_t *le);
extern void crfs_macro_replay_transaction(struct super_block *sb,
