        ...
        if (initsize) {
                if (!sbi->vmmode) {
                        ...
                        if (!fs_initialized) {
                                ...
                                /*
                                 * The file system is already formatted,
                                 * so just map the physical address to 
                                 * the virtual address
                                 */
                                sbi->virt_addr = crfs_ioremap(sb, sbi->phys_addr, initsize);
                                if (!sbi->virt_addr) {
                                        printk(KERN_ERR "ioremap of the crfs image failed\n");
                                        goto out;
                                }

                                /*
                                 * We don't need to map a page to get FS size and then remap it when
                                 * the size is known as originally how PMFS is doing, so I wrap it
                                 * with ifdef 0 for the following code chunk
                                 */
                                /* Do not format storage in the mount option */
