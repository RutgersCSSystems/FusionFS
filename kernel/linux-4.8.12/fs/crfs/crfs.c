...
#ifdef _MACROFS_JOURN
	/* Open log file and log data file */
	op_log_fd = vfio_creatfile_inkernel("/mnt/ram/op_log", (O_CREAT | O_RDWR | O_APPEND), 0666, 0);
	if (op_log_fd < 0) {
		printk(KERN_ALERT "Failed to open op log file\n");
		retval = -EFAULT;
		goto err_creatfs;
	}
	op_log = fget(op_log_fd);

	op_log_data_fd = vfio_creatfile_inkernel("/mnt/ram/op_log_data", (O_CREAT | O_RDWR | O_APPEND), 0666, 0);
	if (op_log_data_fd < 0) {
		printk(KERN_ALERT "Failed to open op log data file\n");
		retval = -EFAULT;
		goto err_creatfs;
	}
	op_log_data = fget(op_log_data_fd);
#endif
...
#ifdef _MACROFS_JOURN
        /* 
         * close op_log file at this point for emulation,
         * otherwise the later umount after injected crash
         * will not succeed.
         */
        fput(op_log);
        fput(op_log_data);

        if (crfss_close_inkernel(op_log, op_log_fd)) {
                printk(KERN_ALERT "%s:%d devfs_close failed\n", __FUNCTION__, __LINE__);
        }

        if (crfss_close_inkernel(op_log_data, op_log_data_fd)) {
                printk(KERN_ALERT "%s:%d devfs_close failed\n", __FUNCTION__, __LINE__);
        }
#endif

	/* Clear injected crash code */
	crash_after_checksum = 0;
