...
static int flowoplib_openreadfile(threadflow_t *, flowop_t *flowop);
...
static int flowoplib_readappend(threadflow_t *threadflow, flowop_t *flowop);
...
static flowop_proto_t flowoplib_funcs[] = {
	...
	{FLOW_TYPE_IO, 0, "openreadclosefile", flowop_init_generic,
	flowoplib_openreadfile, flowop_destruct_generic},
	...
	flowoplib_appendfilerand, flowop_destruct_generic},
        {FLOW_TYPE_IO, FLOW_ATTR_WRITE, "readappend", flowop_init_generic,
        flowoplib_readappend, flowop_destruct_generic},
...
int
fileset_openreadfile(fb_fdesc_t *fdesc, fileset_t *fileset,
    filesetentry_t *entry, int flag, int filemode, int attrs, 
    threadflow_t *threadflow, flowop_t *flowop)
{
	...
	if (attrs & FLOW_ATTR_DIRECTIO)
...
#endif /* HAVE_O_DIRECT */
#if 0 
        if ((fake_fd = open(path, flag | open_attrs, filemode)) < 0) {
                perror("creat");
                return FILEBENCH_ERROR;
        }
        fdesc->fd_num = fake_fd;
        stat(path, &st);
        char *buf = malloc(st.st_size);
        if (pread(fake_fd, buf, st.st_size, 0) != st.st_size) {
        	filebench_log(LOG_ERROR,
        ...
        *read_size = st.st_size;
        ...
        free(buf);
#else
        if ((fake_fd = crfsopen("/mnt/ram/tmp", O_CREAT | O_RDWR, 0666)) < 0) {
                perror("creat");
                return FILEBENCH_ERROR;
        }
        stat(path, &st);
	...
	flowop_beginop(threadflow, flowop);
        if (devfsopenpreadclose(fake_fd, buf, st.st_size, 0, path) != st.st_size) {
        	filebench_log(LOG_ERROR,
		    "Failed to open file %d, %s, with status %x: %s",
		    entry->fse_index, path, entry->fse_flags, strerror(errno));
        ...
        flowop->fo_stats.fs_count+=2;
	flowop_endop(threadflow, flowop, st.st_size);
...
static int
flowoplib_openread_common(threadflow_t *threadflow, flowop_t *flowop, int fd)
{
	filesetentry_t *file;
	char *fileset_name;
	int tid = 0;
	...
	threadflow->tf_fse[fd] = file;
        int read_size = 0;
	err = fileset_openreadfile(&threadflow->tf_fd[fd], flowop->fo_fileset,
	    file, openflag, 0666, flowoplib_fileattrs(flowop), threadflow, flowop);
	...
	if (file->fse_open_cnt == 1)
		file->fse_fileset->fs_idle_files--;
...
static int
flowoplib_openreadfile(threadflow_t *threadflow, flowop_t *flowop)
{
	int fd = flowoplib_fdnum(threadflow, flowop);
	...
	return (flowoplib_openread_common(threadflow, flowop, fd));
}
...
static int
flowoplib_readappend(threadflow_t *threadflow, flowop_t *flowop)
{
        caddr_t iobuf;
        uint64_t appendsize;
        fb_fdesc_t *fdesc;
        ...
        (void) FB_LSEEK(fdesc, 0, SEEK_END);
        ret = devfsreadappend(fdesc->fd_num, iobuf, appendsize);
        if (ret != appendsize) {
                filebench_log(LOG_ERROR,
                    "Failed to write %llu bytes on fd %d: %s",
        ...
        flowop_endop(threadflow, flowop, appendsize * 2);
