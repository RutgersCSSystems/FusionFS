...
#c = pdf('figure1.pdf')
c = canvas('pdf', title='leveldb-readrandom-500k-1thread', dimensions=[100, 70])
t = table(file='readrandom_500k.data')

d = drawable(canvas=c, coord=[21,16], xrange=[-0.5,t.getmax('rownumber')+0.5],
                yrange=[0,40], dimensions=[75, 50])
...
ytitle='Latency (us/op)', ytitlesize=5.5, ylabelfontsize=5.5, yauto=[0,40,10],
ticmajorsize=2, xlabelshift=[0,2], xtitleshift=[0,1])

#grid(drawable=d, x=False, yrange=[10,40], ystep=10, linecolor='lightgrey', linedash=[2,2], linewidth=0.3)
...
L.draw(c, coord=[d.left()+5, d.top()-2], width=4, height=4, fontsize=4, skipnext=3, hspace=1, skipspace=26)
